...
import { getComments, postComment } from "@/services/client/post";
...
function Comment({ id, className }: Props) {
  ...
  const fetchComment = async ({ page = 0 }: any) => {
    try {
      const response = await getComments({ id, params: { page, size: 10 } });
  ...
  const onComment = debounce(async () => {
    if (!content) return;
    setContent("");
    await postComment({ id, body: { rate, content } });
    fetchComment({ page: 0 });
  }, 500);
  ...
  return (
    <div
      className={classNames(
        "p-6 bg-white rounded-xl shadow-Shadow/md",
        className
      )}
    >
      <div className="text-xl font-bold text-Moss/700 mb-3">Bình luận</div>
