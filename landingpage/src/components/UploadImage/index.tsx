...
const UploadImage = ({
  ...
  const image = watch(customKey); // Watch the avatar field
  ...
  const handleFileChange = async (event: ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      setValue(customKey, response.links?.permalink);
    } catch (error) {
      ...
      {image ? (
        <div className="py-4 flex flex-row justify-start items-center gap-4 xl:gap-8 border border-dashed border-white border-b-gray-300">
          ...
          <div className="flex flex-col items-start justify-start gap-2">
            <button
              type="button"
              className="py-3 px-12 rounded-lg shadow-Shadow/xs bg-Moss/400 text-white border-Moss/400 hover:bg-Moss/600 hover:border-Moss/600"
              onClick={() => setValue(customKey, "")}
            >
        ...
        <div className="flex items-center w-full">
          ...
          >
            <div className="w-full py-4 flex flex-row justify-start items-center gap-4 xl:gap-8 border border-dashed border-white border-b-gray-300">
              {showIcon && (
                <ImageIcon className="w-32 h-32" fillColor="#D1D1D6" />
              )}
              <div className="flex flex-col items-start justify-start gap-2">
